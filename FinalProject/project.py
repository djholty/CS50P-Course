...
def main():
    ...
    while True:
        print("""
        Welcome to the ledger.  Here you can select your child and record completed workbooks as
        well as view the list of completed workbooks.
        ...
        if menu1 == '1':
            ...
            cur = conn.cursor()
            cur.execute('''SELECT Children.name FROM Children''')
            items = cur.fetchall()
            conn.close()
            ...
            children = []
            for item in items:
                children.append(item[0].lower())
                
            
            while True:
                for child in children:
                    print(f'Child: {child.capitalize()}')
                child = input("Please Select Your Child from those listed: ")
                child = child.capitalize()
                #check to see if selected child is unavailable
                if child.lower() not in children:
                    print("Invalid input, please select a listed child")
                    continue
                    ...
                    conn = sqlite3.connect('ledgerdb.sqlite')
                    cur = conn.cursor()
                    cur.execute('''SELECT Children.id FROM Children WHERE Children.name = ?''',(child,))
                    ...
                    while True:
                        ...
                        while True:
                            if menu2 == '1':
                                insert_new_completedwkbook(child_id)
                                break
                            elif menu2 == '2':
                                insert_new_transaction(child_id)
                                break
                            elif menu2 == '3':
                                view_completed_workbooks(child)
                                break
                            elif menu2 =='4':
                                get_child_transactions(child)
                                break
                            elif menu2 == '5':
...
def insert_new_completedwkbook(child_id):
    conn = sqlite3.connect('ledgerdb.sqlite')
    cur = conn.cursor()
    date = get_date()
    workbooknum = choose_workbook()

    print()
    print(f'Child ID: {child_id} Date: {date} Workbook: {workbooknum}')
    ...
    while True:
        ...
        try:
            ...
            if len(year) == 4 and len(month) == 2 and len(day) == 2:
                ...
                print("Invalid format, please input as YYYY-MM-DD")
                continue        
        except:
            print('Invalid formatting, please input as YYYY-MM-DD')
            continue
...
def insert_new_transaction(child_id):
    conn = sqlite3.connect('ledgerdb.sqlite')
    cur = conn.cursor()
    date = get_date()
...
def view_completed_workbooks(name):
    ...
    AND Workbooks.id = Members.workbooks_id WHERE Children.name = ? ORDER BY Members.date''', (name,))
...
def get_child_transactions(name):
    conn = sqlite3.connect('ledgerdb.sqlite')
    cur = conn.cursor()

    cur.execute('''SELECT Children.name, Account.date, Account.description, Account.amount
    FROM Children JOIN Account ON Children.id = Account.children_id WHERE Children.name = ?
    ORDER BY Children.name, Account.Date''', (name,))
    ...
    for item in items:
        ...
        print(f'{name}    {date}    {descr:60}    Amount: {amount:6.2f}    Cumulative Total:   {cumtot:.2f}')
